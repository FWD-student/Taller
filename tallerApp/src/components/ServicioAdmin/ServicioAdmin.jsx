...
function ServicioAdmin() {
  ...
  const resetForm = () =>
    setForm({ nombre: "", descripcion: "", precio: "", duracion: "", imagen: "", tipo: "comun" });

  const abrirModal = (servicio = null) => {
    setEditando(servicio);
    setForm(servicio || { nombre: "", descripcion: "", precio: "", duracion: "", imagen: "", tipo: "comun" });
    setModal(true);
  };
  ...
  <div className="contenedor">
    <div className="headerServicios">
      ...
      <button onClick={() => abrirModal()} className="btnAgregar">
        + Nuevo Servicio
      </button>
    </div>
      ...
      {servicios.map((serv) => (
        <div key={serv.id} className="cardServicio">
          ...
          <div className="badgeContainer">
            {serv.tipo === "promocion" && <span className="badgePromocion">Promoción Especial</span>}
          </div>
          <h3>{serv.nombre}</h3>
          <p>{serv.descripcion}</p>
          <p><strong>Precio:</strong> ₡{serv.precio}</p>
          <p><strong>Duración:</strong> {serv.duracion} min</p>
          <p><strong>Tipo:</strong> {serv.tipo === "promocion" ? "Promoción Especial" : "Servicio Común"}</p>
        ...
        <div className="modalContenido">
          ...
          <form onSubmit={guardarServicio}>
            ...
            <select
              ...
              required
            >
              <option value="comun">Servicio Común</option>
